 ...
 */
function wp_dbmanager_test_source_files() {
 ...
 * use, so "does the plugin still enqueue jquery?" would match the changelog
 * note explaining that it does not, and a test asserting on the raw text fails
 * for the wrong reason -- or worse, passes for one.
 *
 * @param string[] $skip Basenames to leave out.
 * @return string
 */
function wp_dbmanager_test_source_code( array $skip = array() ) {
	$code = '';

	foreach ( wp_dbmanager_test_source_files() as $file ) {
 ...
 * @return string
 */
function wp_dbmanager_test_read( $relative ) {
