	...
	if ( ! empty( $_POST['do'] ) ) {
		...
		switch ( $_POST['do'] ) {
			case __( 'Run', 'wp-dbmanager' ):
				...
				$sql_queries2    = isset( $_POST['sql_query'] ) ? trim( wp_unslash( $_POST['sql_query'] ) ) : '';
				$totalquerycount = 0;
				$successquery    = 0;
				if ( $sql_queries2 ) {
					$sql_queries  = array();
...
<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . plugin_basename( __FILE__ ) ) ); ?>">
	...
	<div class="wrap">
		...
		<table class="form-table">
			...
			<tr>
				<td align="center"><input type="submit" name="do" value="<?php esc_html_e( 'Run', 'wp-dbmanager' ); ?>" class="button" />&nbsp;&nbsp;<input type="button" name="cancel" value="<?php esc_html_e( 'Cancel', 'wp-dbmanager' ); ?>" class="button" onclick="javascript:history.go(-1)" /></td>
			</tr>
		</table>
		<p>
